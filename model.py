...
class model(object):
    ...
    def __init__(self,spec,process,source,ppcross,enconv=1.e3,nucfac=1.845):
        self.spec=spec
        self.proc=process
        self.source=source
        self.ppcrs=ppcross
...
w        OUTPUT:
    ...
    def model_val(self,para):
        ...
        ePoints=np.logspace(emin,emax,spectrum.points)*1.e-9 #in GeV#self.data[0]/self.conv
        ...
        if self.finalGamma:
            ...
            return flux/(4.*np.pi*(self.source.dist*const.PARSEC*const.m2cm)**2.),ePoints*self.conv
